...
''' <summary>
''' 予約カレンダー（シアター）で使用する情報取得・更新SQL作成
''' </summary>
''' <remarks>予約カレンダー（シアター）で使用する情報取得・更新のSQLを作成する
''' <para>作成情報：2015/08/06 h.endo
''' <p>改訂情報:</p>
''' </para></remarks>""
Public Class SqlEXTB0101
...
#Region "SQL文(その他利用予約一覧取得)"

    'SQL文(その他利用予約一覧取得)
    Private strSelectEtcUsedYoyakuListSQL As String = _
   "select" & vbCrLf & _
   "   RIYO_DT" & vbCrLf & _
   "  ,START_TIME" & vbCrLf & _
   "  ,END_TIME" & vbCrLf & _
...
#Region "祝祭日取得"
    ...
    Public Function setSelectShukusaijitsu(ByRef Adapter As NpgsqlDataAdapter, _
                                       ...
                                       ByVal dataEXTB0101 As DataEXTB0101) As Boolean
        ...
        Try
            ...
            With Adapter.SelectCommand
                .Parameters(":HOLIDAY_DT").Value = dataEXTB0101.PropStrYear & "/" & dataEXTB0101.PropStrMonth      '祝祭日
            End With
...
#Region "予約一覧取得"
    ...
    Public Function setSelectYoyakuList(ByRef Adapter As NpgsqlDataAdapter, _
        ...
        Try
            ...
            'バインド変数の値を設定
            With Adapter.SelectCommand
                If Mode = YOYAKU_NENGAPPI Then       '利用日
                    .Parameters(":YOYAKU_DT").Value = dataEXTB0101.PropStrYYMMDD
                Else
                    .Parameters(":YOYAKU_DT").Value = dataEXTB0101.PropStrYear & "/" & dataEXTB0101.PropStrMonth
                End If
            End With
...
#Region "キャンセル一覧取得"
    ...
    Public Function setSelectCancelList(ByRef Adapter As NpgsqlDataAdapter, _
        ...
        Try
            ...
            With Adapter.SelectCommand
                If Mode = CANCEL_WAITDATEONLY_NENGAPPI Then
                    .Parameters(":RIYO_DT").Value = dataEXTB0101.PropStrYYMMDD      '利用日
                Else
                    .Parameters(":RIYO_YM").Value = dataEXTB0101.PropStrYear & "/" & dataEXTB0101.PropStrMonth      '利用年月
                End If
...
#Region "キャンセル待ち未確定一覧取得"
    ...
    Public Function setSelectMikakuCancelWaitList(ByRef Adapter As NpgsqlDataAdapter, _
        ...
        Try
            ...
            'バインド変数の値を設定
            With Adapter.SelectCommand
                If Mode = CANCEL_WAITDATEONLY_NENGAPPI Then     '利用日
                    .Parameters(":YOYAKU_DT").Value = dataEXTB0101.PropStrYYMMDD
                Else
                    .Parameters(":YOYAKU_DT").Value = dataEXTB0101.PropStrYear & "/" & dataEXTB0101.PropStrMonth
                End If
...
#Region "その他利用予約一覧取得"
    ...
    ''' <param name="dataEXTB0101"></param>
    ''' <returns>boolean エラーコード  true 正常終了  false	異常終了 </returns>
    ''' <remarks>その他利用予約一覧を取得するSQLの作成
    ''' <para>作成情報：2015/08/07 h.endo
    ''' <p>改訂情報：</p>
    ''' </para></remarks>
    Public Function setSelectEtcUsedYoyakuList(ByRef Adapter As NpgsqlDataAdapter, _
                                       ByRef Cn As NpgsqlConnection, _
                                       ByVal dataEXTB0101 As DataEXTB0101) As Boolean
        ...
        Try
            ...
            'SQL文(SELECT)
            'データアダプタに、SQLのSELECT文を設定
            strSQL = strSelectEtcUsedYoyakuListSQL
            ...
            'バインド変数の値を設定
            With Adapter.SelectCommand
                .Parameters(":RIYO_DT").Value = dataEXTB0101.PropStrYear & "/" & dataEXTB0101.PropStrMonth      '利用日
            End With
...
#Region "休館日、メンテ日一覧取得"
    ...
    Public Function setSelectKyukanMainteList(ByRef Adapter As NpgsqlDataAdapter, _
        ...
        Try
            ...
            'バインド変数の値を設定
            With Adapter.SelectCommand
                If Mode = KYUKANMAINTE_NENGAPPI Then      '利用日
                    .Parameters(":HOLMENT_DT").Value = dataEXTB0101.PropStrYYMMDD
                Else
                    .Parameters(":HOLMENT_DT").Value = dataEXTB0101.PropStrYear & "/" & dataEXTB0101.PropStrMonth
                End If
            End With
...
#Region "休館・メンテ解除"
    ...
    Public Function setDeleteKyukanMainte(ByRef cmd As NpgsqlCommand, _
        ...
        Try
            ...
            '値を設定
            cmd.Parameters(":HOLMENT_DT").Value = dataEXTB0101.PropStrYYMMDD        '対象日
...
#Region "予約制御表登録"
    ...
    Public Function insertYoyakuCtl(ByRef Cmd As NpgsqlCommand, _
                                   ByRef Cn As NpgsqlConnection, _
                                   ByVal dataRiyobi As String) As Boolean
