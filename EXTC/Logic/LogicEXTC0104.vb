...
Public Class LogicEXTC0104

    '変数宣言
    Private sqlEXTC0104 As New SqlEXTC0104          'sqlクラス
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function GetYoyakuData(ByRef dataEXTC0104 As DataEXTC0104) As Boolean
        'ログ出力
        ...
        Try
            ...
            If sqlEXTC0104.setSelectYoyakuData(Adapter, Cn, dataEXTC0104.PropStrYoyakuNo) = False Then
                '異常終了
                Return False
            End If
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function TransData(ByRef dataEXTC0104 As DataEXTC0104, ByVal Table As DataTable) As Boolean
        Try
            With dataEXTC0104
                ...
                .PropStrOnkyoOpe = IIf(DBNull.Value.Equals(Table.Rows(0).Item("ONKYO_OPE_FLG")), "", Table.Rows(0).Item("ONKYO_OPE_FLG"))
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function GetRiyobiData(ByRef dataEXTC0104 As DataEXTC0104) As Boolean
        'ログ出力
        CommonLogic.WriteLog(Common.LogLevel.TRACE_Lv, "START", Nothing, Nothing)
        ...
        Try
            ...
            'SELECT用SQLCommandを作成
            If sqlEXTC0104.setSelectRiyobiData(Adapter, Cn, dataEXTC0104.PropStrYoyakuNo) = False Then
                '異常終了
                Return False
            End If
            ...
            End If
            '値の設定
            TransDataRiyobiList(dataEXTC0104, Table)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function TransDataRiyobiList(ByRef dataEXTC0104 As DataEXTC0104, ByVal Table As DataTable) As Boolean
    ...
    ''' <p>改訂情報:</p>
    ''' </para></remarks>
    Public Function RegYoyakuInfo(ByRef dataEXTC0104 As DataEXTC0104, _
        ...
        Try
            ...
            sqlEXTC0104.registerYoyakuInfo(Cmd, Cn, dataEXTC0104, blnIsUpdate)
            'Cmd.ExecuteNonQuery()
            ...
            If blnIsUpdate = False Then
                dataEXTC0104.PropStrYoyakuNo = strSeqNo
            End If
    ...
    Public Function DeleteYoyakuList(ByVal YoyakuNo As String) As Boolean
        ...
        Try
            ...
            '予約日程表削除処理
            Tsx = Cn.BeginTransaction
            sqlEXTC0104.deleteYoyakuList(Cmd, Cn, YoyakuNo)
            Cmd.ExecuteNonQuery()
    ...
    Public Function InsertYoyakuList(ByVal yoyakuNo As String, _
        ...
        Try
            ...
            For Each dataRiyobi As CommonDataCancel In dataList
                'ログ出力
                sqlEXTC0104.insertYoyakuList(Cmd, Cn, dataRiyobi, yoyakuNo)
                Cmd.ExecuteNonQuery()
            Next
            Tsx.Commit()
            Cn.Close()
    ...
    Public Function DeleteYoyaku(ByVal YoyakuNo As String) As Boolean
        ...
        Try
            ...
            '予約日程表削除処理
            Tsx = Cn.BeginTransaction
            sqlEXTC0104.deleteYoyaku(Cmd, Cn, YoyakuNo)
            Cmd.ExecuteNonQuery()
    ...
    Public Function GetWakuNo(ByVal dataRiyobi As CommonDataCancel) As Integer
        ...
        Try
            ...
            If dataRiyobi.PropIntWakuNo Is Nothing = False And dataRiyobi.PropIntWakuNo > 0 Then
                If sqlEXTC0104.selectWakuNo(Adapter, Cn, dataRiyobi, dataRiyobi.PropIntWakuNo) = True Then
                    Return dataRiyobi.PropIntWakuNo
                End If
            End If
            If sqlEXTC0104.selectWakuNo(Adapter, Cn, dataRiyobi, 1) = True Then
                Return 1
            End If
            If sqlEXTC0104.selectWakuNo(Adapter, Cn, dataRiyobi, 2) = True Then
                Return 2
            End If
            If sqlEXTC0104.selectWakuNo(Adapter, Cn, dataRiyobi, 3) = True Then
                Return 3
            End If
    ...
    Public Function GetSekininshaList(ByVal strRiyoshaCd As String) As ArrayList
        ...
        Try
            ...
            'SELECT用SQLCommandを作成
            If sqlEXTC0104.setSelectSekininName(Adapter, Cn, strRiyoshaCd) = False Then
                '異常終了
                Return list
            End If
    ...
    ''' <p>改訂情報:</p>
    ''' </para></remarks>
    Public Function GetSekininshaMailTel(ByVal dataEXTC0104 As DataEXTC0104) As Boolean
        ...
        Try
            ...
            'SQLの作成・設定
            If sqlEXTC0104.GetSqlSekininshaMailTelData(Adapter, Cn, dataEXTC0104) = False Then
                Return False
            End If
